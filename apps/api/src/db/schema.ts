...
export const paymentMethodEnum = pgEnum('payment_method', ['UPI', 'CARD', 'NET_BANKING', 'WALLET', 'COD']);
...
export const walletTransactionReasonEnum = pgEnum('wallet_transaction_reason', [
  'ORDER_PAYMENT', 'ORDER_REFUND', 'TOP_UP', 'CASHBACK',
]);
export const notificationTypeEnum = pgEnum('notification_type', [
  'ORDER_UPDATE', 'PAYMENT', 'PROMOTIONAL', 'SYSTEM',
]);
...
export const otpVerifications = pgTable('otp_verifications', {
  ...
  index('otp_expires_at_idx').on(t.expiresAt),
]);
...
export const categories = pgTable('categories', {
  ...
  index('category_is_active_idx').on(t.isActive),
]);
...
export const payments = pgTable('payments', {
  id: text('id').primaryKey().default(sql`gen_random_uuid()`),
  orderId: text('order_id').unique().notNull().references(() => orders.id),
  userId: text('user_id').notNull().references(() => users.id),
